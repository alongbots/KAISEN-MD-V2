...
conn.ev.on('messages.upsert', async ({ messages }) => {
   ...
   const isBot = sender === conn.user.id;
    ...
    const { antilink } = await groupDB(['link'], { jid }, 'get');
    ...
    const groupLinkRegex = /chat\.whatsapp\.com\/([0-9A-Za-z]{20,24})/gi;
    ...
    if (action === 'null') {
        await client.sendMessage(jid, {
            text: `⚠️ Link detected from @${sender.split('@')[0]} (no action taken)`,
            mentions: [sender]
        });
        return;
    }
